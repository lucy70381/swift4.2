...
class Ch7_Edit_Actions: UIViewController, UITableViewDataSource, UITableViewDelegate {
...
//        return .none
    ...
    // viewDidLoad 加入以下code 設定下拉更新功能
    // tableView.refreshControl = UIRefreshControl()
    // tableView.refreshControl?.addTarget(self, action: #selector(handleRefresh), for: .valueChanged)
    
    // 若下拉表格後想顯示提示文字 可於viewDidAppear加入此code
    override func viewDidAppear(_ animated: Bool) {
    ...
    /* 7-9 右側按鈕 ----------------------------------------------*/
    // 實作trailingSwipeActionsConfigurationForRowAt
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        // .normal為灰色按鈕
        let moreAction = UIContextualAction(style: .normal, title: "更多") { (rowAction, view, boolValue)  in
            
        }
        // 可自行修改backgroundColor
        moreAction.backgroundColor = UIColor.orange
        
        // .destructive為紅色按鈕
        let delAction = UIContextualAction(style: .destructive, title: "刪除") { (rowAction, view, boolValue) in
            
        }
        
        let swipeActions = UISwipeActionsConfiguration(actions: [delAction, moreAction])
